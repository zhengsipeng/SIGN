        ...
        if (np.max(Human_out[5]) > human_thres) and (Human_out[1] == 'Human'):  # This is a valid human
            ...
            for Object in Test_RCNN[image_id]:
                if (np.max(Object[5] > object_thres)) and not (np.all(Object[2] == Human_out[2])):  # valid object
                    blobs['O_boxes'] = np.array([0, Object[2][0], Object[2][1],
                    ...
                    Pattern, blobs['Gnodes'] = Get_next_sp_with_pose(Human_out[2], Object[2], Pose_nodes,
                                                                     im_shape, Object[5])
                    if posetype == 2:
                        Pattern = Get_next_sp_with_posemap(Human_out[2], Object[2], human_pose, num_joints=17)
                    blobs['semantic'] = Get_next_semantic(Object[4], clsid2cls, objs_bert)
                    blobs['sp'] = Pattern.reshape(1, 64, 64, 2)
    ...
    if len(This_image) == 0:
        ...
        for Human_out in Human_out_list:
            for Object in Object_list:
                blobs['H_boxes'] = np.array(
                    [0, Human_out[2][0], Human_out[2][1], Human_out[2][2], Human_out[2][3]]).reshape(1, 5)
                blobs['O_boxes'] = np.array([0, Object[2][0], Object[2][1], Object[2][2],
                                             Object[2][3]]).reshape(1, 5)
                human_pose = copy.deepcopy(Human_out[6])
                Pose_nodes, pose_none_flag = get_pose_nodes(human_pose, Human_out[2], 1, im_shape)
                Pattern, blobs['Gnodes'] = Get_next_sp_with_pose(Human_out[2], Object[2], Pose_nodes, im_shape,
                                                                 Object[5])
                if posetype == 2:
                    Pattern = Get_next_sp_with_posemap(Human_out[2], Object[2], human_pose, num_joints=17)
                blobs['sp'] = Pattern.reshape(1, 64, 64, 2)
                ...
                prediction_HO = net.test_image_HO(sess, im_orig, blobs)
                temp = list()
                temp.append(Human_out[2])
                temp.append(Object[2])  # Object box
                temp.append(Object[4])  # Object Class:qddddddddfsafsadfdsafsdf
                temp.append(prediction_HO[0][0])  # binary score
                temp.append(Human_out[5])  # Human score
                temp.append(Object[5])  # Object score
                This_image.append(temp)
