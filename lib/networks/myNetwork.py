...
class SPGCN_v2():
    ...
    def res5_part(self, pool5_H, pool5_O, pool_part, is_training, name):
        with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
            ...
            fc7_part = tf.reduce_mean(fc7_part, axis=[1, 2])
        return fc7_H, fc7_O, fc7_part
    ...
    # ======================
    def region_classification(self, fc7_H, fc7_O, sp, is_training,
                                   initializer, name):
        with tf.variable_scope(name) as scope:

            # Visual Part
            vfeature = tf.concat([fc7_H, fc7_O, sp], axis=1)
            vscore = slim.fully_connected(vfeature, self.num_classes,
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_V')
            vprob = tf.nn.sigmoid(vscore, name='cls_prob_v')
            tf.reshape(vprob, [1, self.num_classes])
            ...
            spatial_feature = tf.concat([sp, posemap_fc], axis=1)
            spatial_score = slim.fully_connected(spatial_feature, self.num_classes,
                                                 weights_initializer=initializer,
                                                 trainable=is_training,
                                                 activation_fn=None, scope='cls_score_S')
            spatial_prob = tf.nn.sigmoid(spatial_score, name='cls_prob_spatial')

            # Add
            self.predictions["visual_score"] = vscore
            self.predictions["visual_prob"] = vprob
            self.predictions['spatial_score'] = spatial_score
            self.predictions['spatial_prob'] = spatial_prob
            self.predictions["cls_prob_HO"] = vprob * spatial_prob
            #print self.predictions['cls_prob_HO'].shape

            #if self.binary:
            #    self.predictions['cls_prob_HO'] = self.predictions["cls_prob_binary"] * self.predictions['cls_prob_HO']
    ...
    def build_network(self, is_training):
        ...
        if self.posegraph:
            with tf.variable_scope('sp_gcn') as scope:
                x = self.st_gconv(x, self.A, 64, kernel_size,
        ...
        # ------------
        if self.bodypart:
            partboxes = tf.reshape(self.bodypart, [-1, 5])  # N. 17, 5 -> N*17, 5
            ...
            fc7_part = tf.reshape(fc7_part, [-1, 17, 1024])  # N, 17, 1024
        ...
        fc7_SH, fc7_SO = self.head_to_tail(fc7_H, fc7_O, pool5_SH, pool5_SO, is_training)
        ...
        self.region_classification(fc7_SH, fc7_SO, sp, is_training, initializer, 'classification')
    ...
    def add_loss(self):
        with tf.variable_scope('LOSS') as scope:
            ...
            cls_score_V = self.predictions["visual_score"]
            V_cross_entropy = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(labels=label_HO[:self.H_num, :],
                                                    logits=cls_score_V[:self.H_num, :]))
            self.losses['V_cross_entropy'] = V_cross_entropy

            cls_score_S = self.predictions["spatial_score"]
            S_cross_entropy = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(labels=label_HO[:self.H_num, :],
                                                        logits=cls_score_S[:self.H_num, :]))
            self.losses['S_cross_entropy'] = S_cross_entropy
            loss = (V_cross_entropy + S_cross_entropy)
    ...
    def train_step(self, sess, blobs, lr, train_op):
        ...
        feed_dict = {self.image: blobs['image'], self.head: blobs['head'],
                     ...
                     self.H_boxes: blobs['H_boxes'], self.O_boxes: blobs['O_boxes'], self.spatial: blobs['sp'],
