...
class SIGAN():
    ...
    def binary_discriminator(self, fc7_H, fc7_O, fc7_SH, fc7_SO, sp, is_training, name):
        with tf.variable_scope(name) as scope:
            ...
            fc8_binary_2 = tf.concat([fc8_binary_2[:self.H_num, :], fc8_binary_1[self.H_num:, :]], 0)
            # copy the neg part of fc8_binary_1 and concat
    ...
    def build_network(self, is_training):
        ...
        fc2_H = slim.fully_connected(fc1_H, self.num_fc2, scope='fc2_H')
        ...
        fc2_O = slim.fully_connected(fc1_O, self.num_fc2, scope='fc2_O')
        ...
        if self.use_pm or self.use_bi_pm:
    ...
    def add_loss(self):
        with tf.variable_scope('LOSS') as scope:
            ...
            if self.use_binary:
                ...
                binary_cross_entropy = tf.reduce_mean(
                    tf.nn.weighted_cross_entropy_with_logits(targets=label_binary, logits=cls_score_binary,
                                                             pos_weight=2.2))
            ...
            if self.use_Hsolo:
                ...
                H_cross_entropy = tf.reduce_mean(
                    tf.multiply(tf.nn.sigmoid_cross_entropy_with_logits(
                        labels=label_H[:self.H_num], logits=cls_score_H[:self.H_num]), H_mask[:self.H_num]))  # [:self.H_num]
