...
class SIGAN():
    ...
    def add_loss(self):
        with tf.variable_scope('LOSS') as scope:
            ...
            HO_cross_entropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=label_HO,
                                                                                      logits=cls_score))
            ...
            if self.use_binary:
                ...
                #binary_cross_entropy = tf.reduce_mean(
                #    tf.nn.weighted_cross_entropy_with_logits(targets=label_binary, logits=cls_score_binary, pos_weight=2))
