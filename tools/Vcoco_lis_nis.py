...
object_mask = [[1e-05, 0.85], [0.0015, 0.8], [0.0002, 0.85], [0.001, 0.8], [0.0007, 0.85], [0.0007, 0.85], [0.002, 0.8],
               [1e-04, 0.85], [5e-05, 0.85], [1e-05, 0.85], [0.0007, 0.85], [0.0007, 0.85], [0.0007, 0.85],
               [0.0002, 0.85], [0.0007, 0.85], [1e-05, 0.85], [1e-05, 0.85], [0.01, 0.95], [0.0008, 0.8], [1e-05, 0.85],
               [5e-05, 0.95], [0.0007, 0.85], [0.0007, 0.85], [0.0007, 0.85], [0.00015, 1.0], [5e-05, 0.85],
               [1e-04, 0.9], [0.004, 0.75], [0.0004, 0.9], [0.01, 0.9], [0.01, 0.9], [1e-05, 0.95], [0.005, 0.95],
               [0.0007, 0.85], [1e-05, 0.85], [1e-05, 0.85], [5e-05, 0.95], [1e-05, 0.95], [0.001, 0.95], [0.05, 0.8],
               [0.005, 0.8], [0.1, 0.75], [0.005, 0.8], [0.001, 0.8], [0.007, 0.75], [0.0008, 0.8], [0.0015, 0.85],
               [0.004, 0.75], [0.0007, 0.9], [0.0004, 0.85], [0.00015, 0.8], [0.005, 0.8], [1e-04, 0.8], [0.0035, 0.8],
               [0.002, 0.9], [5e-05, 0.85], [0.005, 0.8], [0.0015, 0.85], [1e-05, 0.85], [0.0025, 0.9], [0.05, 0.75],
               [0.0007, 0.85], [0.0007, 0.85], [1e-05, 0.9], [0.0007, 0.85], [0.05, 0.9], [1e-05, 0.85], [0.0015, 1.0],
               [1e-05, 0.85], [1e-05, 0.85], [0.0007, 0.85], [0.0007, 0.85], [0.0007, 0.85], [0.003, 0.75],
               [0.0007, 0.85], [1e-05, 0.85], [0.0035, 0.9], [0.05, 0.75], [1e-05, 0.85], [1e-05, 0.85]]
...
def apply_prior(Object_class, prediction):
    ...
    if (Object_class != 41) and (Object_class != 40) and (Object_class != 42) and (Object_class != 46):
        # not 'wine glass', 'bottle', 'cup', 'bowl', then the action is impossible to be drink
        prediction[0][0][13] = 0
    ...
    if (Object_class != 77) and (Object_class != 43) and (Object_class != 44):
        # not 'scissors', 'fork', 'knife', then the action is impossible to be cur instr
        prediction[0][0][2] = 0

    if (Object_class != 33) and (Object_class != 30):
        # not 'sports ball', 'frisbee', then the action is impossible to be throw and catch
        prediction[0][0][15] = 0
        prediction[0][0][28] = 0
    ...
    if (Object_class != 14) and (Object_class != 61) and (Object_class != 62) and (Object_class != 60) and (
            Object_class != 58) and (Object_class != 57):
        # not 'bench', 'dining table', 'toilet', 'bed', 'couch', 'chair', then the action is impossible to be lay
        prediction[0][0][12] = 0

    if (Object_class != 32) and (Object_class != 31) and (Object_class != 37) and (Object_class != 38):
        # not 'snowboard', 'skis', 'skateboard', 'surfboard', then the action is impossible to be jump
        prediction[0][0][11] = 0
    ...
    if (Object_class != 47) and (Object_class != 48) and (Object_class != 49) and (Object_class != 50) and (
            ...
            Object_class != 55) and (Object_class != 56):
        # not ''banana', 'apple', 'sandwich', 'orange', 'carrot', 'broccoli', 'hot dog', 'pizza', 'cake',
        # 'donut', then the action is impossible to be eat_obj
        prediction[0][0][9] = 0

    if (Object_class != 43) and (Object_class != 44) and (Object_class != 45):
        # not 'fork', 'knife', 'spoon', then the action is impossible to be eat_instr
        prediction[0][0][16] = 0

    if (Object_class != 39) and (Object_class != 35):
        # not 'tennis racket', 'baseball bat', then the action is impossible to be hit_instr
        prediction[0][0][19] = 0
    ...
    if (Object_class != 2) and (Object_class != 4) and (Object_class != 6) and (Object_class != 8) and (
            ...
            Object_class != 18) and (Object_class != 21):
        # not 'bicycle', 'motorcycle', 'bus', 'truck', 'boat', 'train', 'airplane', 'car', 'horse',
        # 'elephant', then the action is impossible to be ride
        prediction[0][0][5] = 0
    ...
    if (Object_class != 2) and (Object_class != 4) and (Object_class != 18) and (Object_class != 21) and (
            ...
            Object_class != 25):
        # not 'bicycle', 'motorcycle', 'horse', 'elephant', 'bench', 'chair', 'couch', 'bed', 'toilet',
        # 'dining table', 'suitcase', 'handbag', 'backpack', then the action is impossible to be sit
        prediction[0][0][10] = 0
...
def generate_pkl(mode, test_D, test_result, prior_mask, Action_dic_inv, sigmoid_coeff, prior_flag=3):
    ...
    for tr in range(len(test_result)):
        ...
        #    print('wrong')
        #    continue
        # bbox_O = np.array(dic_d['object_box'])
        # class_O = np.array(dic_d['object_class'])
        # det_O = np.array(getSigmoid(sigmoid_coeff, np.array(dic_d['O_det'], dtype='float32')))
        dic_new = dict()
        dic_new['image_id'] = image_id
        dic_new['person_box'] = bbox_H
        dic_new['binary_score'] = score_binary_d
        ...
        for i in range(29):
            ...
            if (i == 3) or (i == 17) or (i == 22) or (i == 27):
                ...
                dic_new[agent_name] = det_H * 0  # prediction_H[0][0][i]
                continue
...
if __name__ == '__main__':
    ...
    if mode == 'best':
        input_D = cfg.ROOT_DIR + '/Results/60000_TIN_VCOCO_D.pkl'  # vcoco default D
    elif mode == 'mode1':
