...
class SPGCN():
    ...
    def head_to_tail(self, fc7_H, fc7_O, pool5_SH, pool5_SO, sp, is_training):
        with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
    ...
    def binary_discriminator(self, fc7_H, fc7_O, fc7_SH, fc7_SO, sp, is_training, name):
        with tf.variable_scope(name) as scope:
            ...
            if not self.bi_posegraph:
                ...
                pool2_flat_pose_map = slim.dropout(pool2_flat_pose_map, keep_prob=self.keep_prob,
                                                   scope='dropout_pose_map_fc')
    ...
    # ======================
    def region_classification_iCAN(self, fc7_H, fc7_O, sp, is_training,
                                   initializer, name):
        with tf.variable_scope(name) as scope:
            coarse_feat = tf.concat([fc7_H[:self.H_num], fc7_O, sp[:self.H_num]], axis=1)

            cls_score_coarse = slim.fully_connected(coarse_feat, self.num_classes,
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_H')
            cls_prob_coarse = tf.nn.sigmoid(cls_score_coarse, name='cls_prob_H')
            tf.reshape(cls_prob_coarse, [1, self.num_classes])

            self.predictions["cls_score_coarse"] = cls_score_coarse
            self.predictions["cls_prob_coarse"] = cls_prob_coarse
            self.predictions['cls_prob_HO'] = self.predictions['cls_prob_coarse']
            if self.binary:
    ...
    def build_network(self, is_training):
        ...
        fc9_SH, fc9_SO, fc9_SHsp = self.head_to_tail(fc7_H, fc7_O, pool5_SH, pool5_SO, sp, is_training)

        # --------------
        # Classification
        # --------------
        if self.binary:
            fc9_binary = self.binary_discriminator(fc7_H, fc7_O, fc9_SH, fc9_SO, sp, is_training, 'fc_binary')
            self.binary_classification(fc9_binary, is_training, initializer, 'binary_classification')

        self.region_classification_iCAN(fc9_SH, fc9_SO, fc9_SHsp,
    ...
    def add_loss(self):
        with tf.variable_scope('LOSS') as scope:
            ...
            cls_score_coarse = self.predictions["cls_score_coarse"]
            cls_score_binary = self.predictions["cls_score_binary"]
            coarse_cross_entropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=label_HO[:self.H_num, :],
                                                                                     logits=cls_score_coarse[:self.H_num,
                                                                                            :]))
            ...
            loss = coarse_cross_entropy + binary_cross_entropy
            self.losses['coarse_cross_entropy'] = coarse_cross_entropy
