...
class SPGCN():
    ...
    # ======================
    def region_classification(self, fc7_H, fc7_O, fc7_SH, fc7_SO, sp, is_training, initializer, name):
        with tf.variable_scope(name) as scope:
            #fc_HOsp = tf.concat([fc7_SH, fc7_SO], axis=1)
            fc_HOsp = fc7_SO
            ...
            if self.H:
                cls_score_H = slim.fully_connected(fc7_SH, self.num_classes,
                                                   weights_initializer=initializer,
                                                   trainable=is_training,
    ...
    def st_gconv(self, input, A, in_channels, out_channels, kernel_size, name):
        with tf.variable_scope(name) as scope:
            ...
            if not self.posegraph_att:
                ...
                query = tf.reshape(input, [-1, self.num_nodes, in_channels])  # N, k, c
                key = tf.transpose(query, perm=[0, 2, 1])  # N, c, k
                mul = tf.einsum('nkc, ncv->nkv', query, key)  # N, k, k
                ori_A = tf.tile(tf.expand_dims(self.ori_A, 0), (self.H_num, 1, 1))
                att = tf.multiply(mul, ori_A)  # N, k, k
                norm_att = att * tf.rsqrt(tf.cast(in_channels, dtype=tf.float32))  # N, k, k
                att_score = tf.nn.softmax(norm_att, axis=2)  # N, k, k
                att_score = tf.tile(tf.expand_dims(att_score, 1), (1, kernel_size, 1, 1))  # N, 1, k, k -> N, 3, k, k
                att_A = tf.einsum('nkvw, kvw->nkvw', att_score, A)
    ...
    def pose_pooling(self, x):
        ...
        # '''
        # pool0
        #x = slim.conv2d(x, 256, kernel_size=(1, 1), stride=1, padding='VALID', activation_fn=tf.nn.relu)
        #allnodes = tf.reshape(slim.avg_pool2d(x, [1, self.num_nodes]), [-1, 256])  # [N, 1, 19, 256]
        ...
        allnodes = slim.fully_connected(x, 256, scope='fc_spgcn')
        '''
    ...
    def pose_attention(self, nodes_feat, semantic):
        ...
        ws1 = slim.fully_connected(semantic, 512, scope='fc_ws1')  # N, 256
        ws1 = slim.dropout(ws1, keep_prob=self.keep_prob)
        ws2 = slim.fully_connected(ws1, 256, scope='fc_ws2')
        ws2 = slim.dropout(ws2, keep_prob=self.keep_prob)
        score = tf.einsum('ntkc, nc->ntk', pose, ws2)  # [N, 1, 17]
    ...
    def build_network(self, is_training):
        ...
        if self.posegraph:
            with tf.variable_scope('sp_gcn') as scope:
                # input_channel -> 64, input=input_channel*spatial_channel
        ...
        self.region_classification(fc7_H, fc7_O, fc7_SH, fc7_SO, sp, is_training, initializer, 'classification')
        ...
        with tf.variable_scope('LOSS') as scope:
            ...
            if self.H:
                ...
                H_cross_entropy = tf.reduce_mean(
                    tf.multiply(tf.nn.sigmoid_cross_entropy_with_logits(labels=label_H, logits=cls_score_H[:self.H_num]), H_mask))
                self.losses['H_cross_entropy'] = H_cross_entropy
                loss += 2 * H_cross_entropy
            else:
                loss += 2 * HO_cross_entropy
                self.losses['H_cross_entropy'] = HO_cross_entropy
    ...
    def test_image_H(self, sess, image, blobs):
        feed_dict = {self.image: image, self.Hsp_boxes: blobs['H_boxes'],
                     self.head: blobs['head'], self.H_num: blobs['H_num']}
